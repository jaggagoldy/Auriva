...
- **P3 ✅** Practice Setup module (`/clinic` → Settings): `StorageService` abstraction (local `.uploads` today, swappable to S3/R2 — UI only holds urls) + `POST /api/clinic/uploads` + `GET /api/files/[key]`; `practice-profile` aggregate (`GET/PATCH /api/clinic/profile`); full UI — doctor (photo, specialty, qualifications, experience, languages, registration, fees, bio), clinic (logo, cover, about, address, contacts, email, website), hours, facilities, gallery, documents, social. Services = existing Treatments tab. Verified upload→save→persist, 0 errors.
- **P4** multi-service billing — DEFERRED.
- **P5 ✅** Diagnostics (replaces removed Labs — recommendation only): catalog (30 tests × 6 categories + packages, acronym search, prep notes); `TestRecommendation` model; consult **DiagnosticsSelector** (search/category/packages/checkboxes) → creates patient recommendations; **patient Health Vault** (Records → Tests): prep instructions + status tracker (Pending→Booked→Completed→Report uploaded) + report upload. Verified e2e.
- **P6 ✅** Clinical Templates: `ClinicalTemplate` model (SOAP + advice/exercises/follow-up, favourite; clinic_id for future sharing); `/api/clinic/templates` CRUD; consult **Templates** bar (quick-apply chips, save-current, manage: edit/favourite/delete); apply fills/append consult fields. Verified e2e.
- **Self-review:** whole-project `tsc` clean; full test suite **385/385**; my files eslint-clean (31 pre-existing `set-state-in-effect` errors remain only in untouched out-of-scope `/staff` + `/admin` multi-clinic files).
